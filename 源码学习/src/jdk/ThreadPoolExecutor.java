...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     * below).
     */
    //前3位存储线程池运行状态  后29位保存worker数量
    private final AtomicInteger ctl = new AtomicInteger(ctlOf(RUNNING, 0));
    private static final int COUNT_BITS = Integer.SIZE - 3;
    private static final int CAPACITY   = (1 << COUNT_BITS) - 1;

    // runState is stored in the high-order bits
    private static final int RUNNING    = -1 << COUNT_BITS;//101   高三位
  ...
  //获得线程池运行状态
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        retry:   //下面跟for循环    continue或者break跳到标记的那层for循环。不是当前的for循环
        for (;;) {
            int c = ctl.get();
            //获得线程池状态
            int rs = runStateOf(c);
            ...
            for (;;) {
                ...
                //工作线程大于最大返回或超过线程池数量返回失败
                if (wc >= CAPACITY ||
                        wc >= (core ? corePoolSize : maximumPoolSize))
                    return false;
                //CAS worker数量+1
                if (compareAndIncrementWorkerCount(c))
                    break retry;//成功跳出for循环
       ...
       ThreadPoolExecutor.Worker w = null;
        try {
            ...
            //启动失败，将worker移除worker集合，并且workercount-1
            if (! workerStarted)
                addWorkerFailed(w);
        }
     ...
     */
    //如果任务队列中没有任务，worker数量大于核心线程数，则需要销毁
    // 如果没有任务，但是worker数量没有超出核心线程数量，则停留在for循环中不停轮询，直到队列中有新的任务进来
    ...
    /**
     *
     * @param corePoolSize  核心线程数  没有设置timeout 将不会被销毁
     * @param maximumPoolSize 最大线程数
     * @param keepAliveTime 非核心线程存活时间（空闲状态）
     * @param unit 存活时间单位
